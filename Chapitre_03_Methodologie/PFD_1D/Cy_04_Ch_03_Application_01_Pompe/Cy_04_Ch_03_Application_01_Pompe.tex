% "{'chapitre':'dyn_1d','classe':('PSI'),'type':('application'),'titre':'Pompe à plateau', 'source':'C. Gamelon et P. Dubois','comp':('DYN-02','DYN-05','DYN-06','C1-05','C2-08','C2-09'),'corrige':True}"
...
Le ressort \textbf{(r)}, de raideur $k$, est toujours comprimé. Pour $\theta = -\dfrac{\pi}{2}$, l'effort de compression est égal à $\vect{F_0}=-F_0\vect{z_0}$.\sidenote{On pourrait montrer que $\vect{F_r} = -F_r \vect{z} = -\left(F_0+k\left(z+e-R\right) \right)\vz{0}$.}
...
%\large{\textbf{TODO  : remplacer l'expression de $\vect{F_r}$ par  $\vect{F_r} = -F_r \vect{z} = -\left(F_0+k\left(z+e-R\right) \right)\vz{0}$}}
\item Ressort: $\torseurstat{T}{\text{Ressort}}{2}
=\torseurl{-\left(F_0+k\left(z+e-R\right) \right)\vz{0}}{\vect{0}}{A}$.
...
\textbf{Bilan :}
$$C_m-\left(\left(e\cos\theta+R\right)Y_{21} +e \left( -F_h-\left(F_0+k\left(z+e-R\right) \right)-m_2g-m_2\ddot{z}\right)\sin\theta\right) =I_1\ddot{\theta}.$$

On a alors :
$$C_m-\left(\left(e\cos\theta+R\right)Y_{21} -e \left( F_h+\left(F_0+k\left(z+e-R\right) \right)+m_2g-em_2\left( \ddot{\theta}\sin\theta+\dot{\theta}^2\cos\theta\right)\right)\sin\theta\right) =I_1\ddot{\theta}.$$


\textbf{Bilan sans frottement :}
$$C_m+e\left(
F_h+\left(F_0+k\left(z+e-R\right) \right)+m_2g
