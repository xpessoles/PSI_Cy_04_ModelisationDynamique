...
Dynamique du véhicule -- Chariot élévateur à bateaux $\quad$ \ifnormal $\star$ \else \fi \ifdifficile $\star\star$ \else \fi \iftdifficile $\star\star\star$ \else \fi
-- \ifprof Corrigé \else Sujet \fi}
...
\ifprof
\else
La digure ci-dessous donne le paramétrage retenu pour l’étude. On note $\rep{1}=\repere{O}{x_1}{y_1}{z_1}$ le repère lié au chariot. Le plan $\left(O,\vect{x_1},\vect{z_1} \right)$ est un plan de symétrie matériel pour le chariot.
\marginnote{
On note : 
\begin{itemize}
...
L’objectif de cette partie est de mettre en évidence l’intérêt d’un dispositif permettant d'éviter le basculement frontal du chariot élévateur.  
...
\begin{itemize}
\item Poids du bateau : $\torseurstat{T}{\text{pes}}{B}=\torseurl{-m_Bg\vect{z}}{\vect{0}}{G_B}=\torseurl{-m_Bg\vect{z}}{m_Bg\vect{y}\left(x_{G_B}-\dfrac{2L}{3} \right)+Em_Bg\vect{x}}{I_4}$. 
...
\ifprof
\begin{corrige}
On reste à la limite du basulement  où $N_1 = T_1=0$. 
Le glissement est traduit par le TRD. On a donc :
\begin{itemize}
\item TRD suivant $\vect{x_1}$ :$T_4 = \left(M+m_B\right) \text{dec}_x$;
\item TRD suivant $\vect{z_1}$ :$N_4= \left(M+m_B\right)g$.
\end{itemize}
Il y a adhérence tant que $T_4 <f N_4$, c'est à dire  : $\left(M+m_B\right) \text{dec}_x < f\left(M+m_B\right)g$ et  $\text{dec}_x < f g$; donc si $f>\dfrac{\text{dec}_x}{g}$, le basculement aura lieu avant le glissement.
\end{corrige}
\else
\fi
...
\begin{solution}
\begin{enumerate}
\item $x_{G_C}=-\dfrac{m_T}{m_C}x_{G_T}-\dfrac{m_1}{m_C}x_{G_1}$
\item PFD : \begin{itemize}
\item TRD suivant $\vect{x_1}$ :$-\left(M+m_B\right) \text{dec}_x=-\sum\limits_{i=1}^{4} T_i$;
\item TRD suivant $\vect{y_1}$ :$0=0$;
\item TRD suivant $\vect{z_1}$ :$0=\sum\limits_{i=1}^{4} N_i-\left(M+m_B\right)g$.
\item TMD en $I_4$ suivant $\vect{x_1}$ :$0=Em_B g+EM g-2EN_2-2EN_3 $;
\item TMD en $I_4$ suivant $\vect{y_1}$ :$-m_B \text{dec}_x \left(z_{G_B}+h\right)-M\text{dec}_x h=L\left(N_1+N_2 \right)+m_B g\left(x_{G_B}-2\dfrac{L}{3}\right)-\dfrac{M g2L}{3} $;
\item TMD en $I_4$ suivant $\vect{z_1}$ :$ m_B \text{dec}_x E-M\text{dec}_x E=-2ET_2-2ET_3$.
\end{itemize}
\item .
\item $\text{dec}_x  = -g\dfrac{m_B \left(3x_{G_B}-2L\right)-M 2L}{3m_B  \left(z_{G_B}+h\right)+3M h}$
\end{enumerate}
\end{solution}
\fi
