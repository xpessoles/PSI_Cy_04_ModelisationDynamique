...
\item l’action mécanique du câble sur le spreader est notée \sidenote{Aide : cette action correspond également à l'action dans la liaison pivot entre 2 et 3.} $\vectf{2}{3} =F_{23} \vect{z_2}$.
\end{itemize}
...
\textbf{Calcul de $\vect{FG_3}\wedge \vectrd{3}{0}$}


On a $\left(\vect{FG_3}\wedge \vectrd{3}{0}\right)\cdot \vect{x_0} = \square$

$ = \left(-h_3\vz{3} \wedge \left(M_3\left(\ddot{y}_{ch}(t) \vect{y_0}+\ell_2\ddot{\theta} \vect{y_2}+h_3\left(\ddot{\theta}+\ddot{\beta}\right) \vect{y_3}
+   
\ell_2\dot{\theta}^2 \vect{z_2}  +h_3\left(\dot{\theta}+\dot{\beta}\right)^2\vect{z_3}\right)\right)\right)\cdot \vect{x_0} $ 

$ = -h_3\left(M_3\left(\ddot{y}_{ch}(t)\vz{3} \wedge  \vect{y_0}+\ell_2\ddot{\theta}\vz{3} \wedge  \vect{y_2}+h_3\left(\ddot{\theta}+\ddot{\beta}\right) \vz{3} \wedge \vect{y_3}
+   
\ell_2\dot{\theta}^2\vz{3} \wedge  \vect{z_2} \right)\right)\cdot \vect{x_0} $ 

$ = M_3h_3\left(\ddot{y}_{ch}(t)\cos\left(\theta+\beta\right)-\ell_2\ddot{\theta}  \cos \beta  -h_3\left(\ddot{\theta}+\ddot{\beta}\right)    
- \ell_2\dot{\theta}^2  \sin \beta  \right) $ 

$-h_3M_3g \sin\left(\beta+\theta\right) = A_3 \left(\ddot{\theta}+\ddot{\beta} \right) +M_3h_3\left(\ddot{y}_{ch}(t)\cos\left(\theta+\beta\right)-\ell_2\ddot{\theta}  \cos \beta  -h_3\left(\ddot{\theta}+\ddot{\beta}\right)    
- \ell_2\dot{\theta}^2  \sin \beta  \right)$


\textbf{Equation de mouvement}

$-h_3 \vect{z_3} \wedge \left(-M_3g \vect{z_0} \right) \cdot \vect{x_0} =A_3 \left(\ddot{\theta}+\ddot{\beta} \right) +  \square$
$\Leftrightarrow -h_3M_3g \sin\left(\beta+\theta\right) = A_3 \left(\ddot{\theta}+\ddot{\beta} \right) +\square$

%$\Leftrightarrow - M_3gh_3 \sin\left( \beta + \theta\right) =A_3 \left(\ddot{\theta}+\ddot{\beta} \right) + M_3h_3\left(\ddot{y}_{ch}(t)\sin\left(\theta+\beta\right)-\ell_2\ddot{\theta}  \sin \beta  -h_3\left(\ddot{\theta}+\ddot{\beta}\right)    
%- \ell_2\dot{\theta}^2  \sin \beta  \right) $.

\textbf{Linéarisation}

$\Leftrightarrow -g M_3 h_3\left(\beta+\theta\right) = M_3 h_3\left(\ddot{y}_{ch}(t) -\ell_2\ddot{\theta}  -h_3\left(\ddot{\theta}+\ddot{\beta} 
  \right)\right) + A_3 \left(\ddot{\theta}+\ddot{\beta} \right)$
...
$ -g \theta  =  \ddot{y}_{ch}(t) +\ell_2\ddot{\theta} +h_3\left(\ddot{\theta}+\ddot{\beta}\right)  $.
...
\item  $-g M_3 h_3\left(\beta+\theta\right) = M_3 h_3\left(\ddot{y}_{ch}(t) -\ell_2\ddot{\theta}  -h_3\left(\ddot{\theta}+\ddot{\beta} 
  \right)\right) + A_3 \left(\ddot{\theta}+\ddot{\beta} \right)$
...
-h_3\left(\dot{\theta}+\dot{\beta}\right)^2\sin\beta \right)$
%\item $- M_3gh_3 \sin\left( \beta + \theta\right) =A_3 \left(\ddot{\theta}+\ddot{\beta} \right)$
\item $-h_3M_3g \sin\left(\beta+\theta\right) = $

$M_3h_3\left(\ddot{y}_{ch}(t)\cos\left(\theta+\beta\right)-\ell_2\ddot{\theta}  \cos \beta  -h_3\left(\ddot{\theta}+\ddot{\beta}\right)    
- \ell_2\dot{\theta}^2  \sin \beta  \right)+A_3 \left(\ddot{\theta}+\ddot{\beta} \right)$
\end{enumerate}
\end{solution}
\fi
\fi
